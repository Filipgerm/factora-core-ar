...
function ERPSuccessContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { t } = useI18n();
  const [status, setStatus] = useState<"pending" | "success">("pending");

  const { pushWithParams, goToNextStep } = useOnboardingRouting("erp-success");

  // Add ERP integration to localStorage
  useEffect(() => {
    const erpName = searchParams.get("erp");
    if (erpName) {
      // Create an ERP ID from the name (e.g., "Soft1" -> "erp:soft1")
      const erpId = `erp:${erpName.toLowerCase().replace(/\s+/g, "-")}`;
      addIntegration("erp", erpId, erpName);
    }
  }, [searchParams]);
  ...
  useEffect(() => {
    if (status === "success") {
      const redirectTimer = setTimeout(() => {
        ...
        if (isDashboard) {
          // Return to dashboard integrations, keeping params intact
          pushWithParams("/integrations");
        } else {
          // Let the Elite Engine decide the next step (Terms, Platform, etc.)
          goToNextStep();
        }
      }, 1000);
  ...
  }, [status, searchParams, pushWithParams, goToNextStep]);
              ...
              {status === "pending" ? (
                ...
                >
                  ...
                  <h1 className="text-2xl font-bold text-gray-900 mb-2">
                    {t("onboarding.erp_success.pending_title")}
                  </h1>
                  <p className="text-gray-600 text-sm">
                    {t("onboarding.erp_success.pending_subtitle")}
                  </p>
                </motion.div>
              ) : (
                <motion.div
                ...
                >
                  ...
                  <h1 className="text-2xl font-bold text-gray-900 mb-2">
                    {t("onboarding.erp_success.success_title")}
                  </h1>
                  <p className="text-gray-600 text-sm">
                    {t("onboarding.erp_success.success_subtitle")}
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
...
export default function ERPSuccessPage() {
  return (
    <Suspense fallback={<ERPSuccessLoading />}>
      <ERPSuccessContent />
    </Suspense>
  );
}
