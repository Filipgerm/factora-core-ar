...
function PlatformConsentContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { t } = useI18n();
  const platformNameParam = searchParams.get("platform");
  const platformName = platformNameParam || "Your Platform";
  ...
  const handleTestConnection = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (platformNameParam) queryParams.set("platform", platformNameParam);

        const queryString = queryParams.toString();
        const redirectUrl = `/onboarding/platform-success${queryString ? `?${queryString}` : ""
          }`;
        ...
        <div className="px-6 py-5 bg-brand-surface text-white">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold">
              {t("onboarding.platform_consent.title", {
                platform: platformName,
              })}
            </h1>
          </div>
          <p className="text-white/80 mt-1">
            {t("onboarding.platform_consent.subtitle")}
          </p>
        </div>
...
function PlatformConsentLoading() {
  return (
    <div className="flex w-full h-full items-center justify-center bg-gradient-to-br from-brand-page-bg-start to-brand-page-bg-end px-4 py-8">
...
export default function PlatformConsentPage() {
  return (
    <Suspense fallback={<PlatformConsentLoading />}>
      <PlatformConsentContent />
    </Suspense>
  );
}
