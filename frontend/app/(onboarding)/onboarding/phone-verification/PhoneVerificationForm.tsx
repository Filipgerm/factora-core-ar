...
import { DUMMY_PHONE_CODE } from "@/app/lib/dummyCodes";
...
export default function PhoneVerification() {
  ...
  const handleCodeChange = (index: number, value: string) => {
    ...
    if (value && index === 3) {
      ...
      setIsVerifying(true);
      setTimeout(() => {
        const isCorrect = candidate === DUMMY_PHONE_CODE;
  ...
  return (
    ...
    >
      <div className="w-[500px] space-y-6 md:space-y-8">
        ...
        <AnimatedFormHeader title={t("onboarding.phone_verification.header")} />
        ...
        >
          {secondsLeft > 0 ? (
            <p className="text-xs text-brand-primary md:text-sm">
              {t("onboarding.phone_verification.resend_wait", {
                seconds: secondsLeft,
              })}
            </p>
          ) : (
            <p className="text-xs text-gray-600 md:text-sm">
              ...
              >
                {t("onboarding.phone_verification.resend_ready")}
              </span>
            </p>
          )}
        </motion.div>
