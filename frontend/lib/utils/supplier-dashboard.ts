...
// random time between 10 days and 1 hour ago
...
export const buildNotifications = (
  ...
  statusEvents: AnyNotification[];
  alertEvents: AnyNotification[];
  requestEvents: AnyNotification[];
} => {
  const prevById = new Map(
    prev.map((c) => [String(c.id ?? c.email ?? c.name), c])
  );

  const statusEvents: AnyNotification[] = [];
  let alertEvents: AnyNotification[] = [];
  const requestEvents: AnyNotification[] = [];
    ...
    for (const a of currentAlerts) {
      if (!previousAlerts.includes(a)) {
        ...
        alertEvents.push({
          ...
          read: false,
          alertType,
          details: getRiskSeverity(alertType), // Show risk severity level
        });
      }
    }
  ...
  alertEvents = alertEvents.slice(0, 3); // Limit alerts to 3 entities. TO BE REMOVED FOR COMPLETE DYNAMIC RENDERING
